	...
	public void addDatasetFromFunction(LinearPieceWiseFunction function) {
		...
		for (int i = 1; i < points.size(); i++) {
			...
			if (points.get(i).getLeft().equals(points.get(i - 1).getLeft())) {
				...
				//If this is the last point of the function and it created a discontinuity, also big circle
				if(i==points.size()-1) {
					renderer.setSeriesShape(seriesIdx, circle);
				}
		...
		lineChart = ChartFactory.createXYLineChart("", "", "", dataset, PlotOrientation.VERTICAL, false, true, false);
